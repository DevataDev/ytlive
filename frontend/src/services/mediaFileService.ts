...
export const fetchMediaFiles = async (streamId: string): Promise<MediaListResponse> => {
  const session = await getSession();
  const response = await api.get<MediaListResponse>(`/api/streams/${streamId}/media`, {
    headers: { 'Authorization': `Bearer ${session?.user?.backendToken}` }
  });
  return response;
};

export const uploadMediaFile = async (streamId: string, data: MediaFileUploadData): Promise<MediaFileData> => {
  const session = await getSession();
  ...
  const response = await api.post<MediaFileData>(`/api/streams/${streamId}/media`, formData, {
    headers: {
